...
func (g *GitHubProcessor) processGitHubWithTemplate(target string, data template.Data, templateName string) error {
	...
	switch action {
	case "comment":
		// Create a proper JSON document for the GitHub comment
		...
		// Marshal to JSON
		jsonBytes, err := json.Marshal(commentPayload)
		if err != nil {
			log.Error("Failed to create JSON payload", logger.Fields{
				"template": templateName,
				"error":    err.Error(),
			})
			return errors.Wrap(errors.CategoryApplication, err, "Failed to create GitHub JSON payload")
		}
	...
	case "issue":
		// Create a proper JSON document for the GitHub issue
		...
		// Marshal to JSON
		jsonBytes, err := json.Marshal(issuePayload)
		if err != nil {
			log.Error("Failed to create JSON payload", logger.Fields{
				"template": templateName,
				"error":    err.Error(),
	...
	case "pr":
		// Create a proper JSON document for the GitHub PR
		title := fmt.Sprintf("%s - %s", data.PromptName, data.Timestamp.Format("2006-01-02"))
		body := fmt.Sprintf("## Generated by CronAI\n\n"+
			"**Model**: %s\n"+
			"**Timestamp**: %s\n"+
		...
		// Marshal to JSON
		jsonBytes, err := json.Marshal(prPayload)
		if err != nil {
			log.Error("Failed to create JSON payload", logger.Fields{
				"template": templateName,
				"error":    err.Error(),
